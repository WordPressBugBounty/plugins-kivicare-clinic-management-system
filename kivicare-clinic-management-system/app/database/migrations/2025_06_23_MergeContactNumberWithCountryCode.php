...
class MergeContactNumberWithCountryCode extends KCAbstractMigration {
    public function run() {
        ...
        $wpdb->query(" UPDATE {$clinics_table} SET telephone_no =  CASE  WHEN country_calling_code IS NOT NULL AND country_calling_code != ''  THEN CONCAT(country_calling_code, ' ', telephone_no) ELSE telephone_no END "); 
        ...
        foreach ($users_with_basic_data as $user_data) {
            ...
            if (is_array($basic_data) && isset($basic_data['mobile_number'])) {
                ...
                if (!empty($country_code)) {
                    $basic_data['mobile_number'] = $country_code . ' ' . $basic_data['mobile_number'];
