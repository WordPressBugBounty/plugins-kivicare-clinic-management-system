...
wp.blocks.registerBlockType( 'kivi-care/popup-book-appointment-widget', {
    title: 'Kivi Appointment Button Widget',
    ...
    edit: function( props ) {
        ...
        if(Array.isArray(clinicList1New)) {
            clinicList1New.forEach(function(value) {
        ...
        function getShortCode(props) {
            ...
            var short_code = '[kivicareBookAppointmentButton' + (attrs.length ? ' ' : '') + attrs.join(' ') + ']';
        ...
        function onChangeClinic( clinicId )
        {
            ...
            if(DataListNew.proActive){
                const clinic_doctors = mappingDataNew.filter(item => item.clinic_id == clinicId);
            ...
            clinic_vise_doctors.forEach(function (doctor) {
                const doctorId = doctor.ID;
                const doctorName = doctor.data?.display_name;
                doctorIDListNew += (doctorIDListNew ? ',' : '') + doctorId;
                doctorListNew += `${doctorName}(ID:${doctorId}), `;
            });

            props.setAttributes( { doctorList: doctorListNew } );
            props.setAttributes( { doctorId: doctorIDListNew } );
        }
        ...
        function resetShortCode(){
            ...
            props.setAttributes( { doctorId: '' } );
            props.setAttributes( { doctorList: '' } );
        }
                    ...
                    el(
                        ...
                        el(
                            ...
                            (props.attributes.clinicId == 0 ? '' : el(
                                    ...
                                    {
                                        label: 'Doctor list',
                                        value: props.attributes.doctorList,
                                        readOnly: true
                                    }
                                )
                            ),
                            (props.attributes.clinicId == 0 ? '' : el(
                                    TextareaControl,
                                    {
                                        ...
                                        placeholder: props.attributes.doctorId,
                                        value: props.attributes.doctorId,
